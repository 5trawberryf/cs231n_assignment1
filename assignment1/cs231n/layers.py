...
def affine_backward(dout, cache):
    ...
    x_data = x.reshape(N,-1)
    dw = x_data.transpose().dot(dout)
    dx_data = dout.dot(w.transpose())
    dx = dx_data.reshape(x.shape) 
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def svm_loss(x, y):
    ...
    dx = np.ones(margin.shape) * mask
    counts_C = np.sum(dx,1) # num of right c need to be -,(N,)
    dx[np.arange(num_train),y] -= counts_C
    dx /= num_train
    loss /= num_train
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
